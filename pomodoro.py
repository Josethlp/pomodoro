...
class PomodoroTimer:
    def __init__(self):
        ...
        self.thimer_label = tk.Label(self.root, text="",
                                     font=("TkDefaultFont", 40))
        self.thimer_label.pack(pady=20)
        ...
        if self.is_running:
            if self.is_work_time:
                ...
                if self.break_time == 0:
                    ...
                    messagebox.showinfo("Work Tie", "Get back to work!")

            minutes, seconds = divmod(self.work_time if self.is_work_time else self.break_time, 60)
            self.thimer_label.config(text="{:02d}:{:02d}".format(minutes, seconds))
